...
<div class="max-w-2xl mx-auto">
    <h1 class="text-2xl font-serif mb-6">Edit FAQ</h1>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6">
        <form method="POST" action="{{ route('admin.faqs.update', $faq) }}">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="question" value="{{ old('question', $faq->question) }}" required class="w-full border border-sage-200 rounded-lg px-4 py-2">
            ...
            <div class="mb-4">
                ...
                <textarea name="answer" rows="5" required class="w-full border border-sage-200 rounded-lg px-4 py-2">{{ old('answer', $faq->answer) }}</textarea>
                @error('answer') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
            </div>

            <div class="mb-4">
                <label class="block text-sage-700 mb-2">Urutan</label>
                <input type="number" name="order" value="{{ old('order', $faq->order) }}" class="w-32 border border-sage-200 rounded-lg px-4 py-2">
            </div>
